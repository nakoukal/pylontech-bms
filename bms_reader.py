...
# --- UPRAVENÁ PARSOVACÍ FUNKCE (PRO VŠECHNA DATA) ---
...
# --- UPRAVENÁ FUNKCE PRO VŠECHNY SENZORY ---
def publish_ha_discovery(client):
    print("Publishing FULL MQTT Discovery configuration (in English)...")
    device_info = {"identifiers": [DEVICE_UNIQUE_ID], "name": DEVICE_NAME, "manufacturer": "Pylontech"}
    
    # Definice pro všechny souhrnné senzory
    summary_sensors = {
    ...
    # Definice pro senzory každého článku
    cell_sensors = {
...
# --- MAIN SCRIPT (s upraveným odesíláním) ---
...
try:
    ...
    while True:
        ...
        if data and data.get('summary') and data.get('footer') and data.get('cells'):
            ...
            # Publikace všech souhrnných dat
            ...
            for cell in data['cells']:
                # Iterujeme přes všechny klíče v dictionary pro daný článek (kromě 'id')
