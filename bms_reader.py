...
# --- OPRAVA: Použita PŘESNĚ vaše funkční verze ---
...
def parse_bms_data(raw_data_str):
    ...
    if len(header_parts) >= 8:
        bms_data['summary'] = {'voltage': safe_float(header_parts[0])/1000.0, 'current': safe_float(header_parts[1])/1000.0, 'avg_temperature': safe_int(header_parts[2])/1000.0, 'capacity': safe_int(header_parts[3]), 'status': header_parts[4], 'voltage_status': header_parts[5], 'current_status': header_parts[6], 'temperature_status': header_parts[7]}
    for i, line in enumerate(cell_lines):
        cell_parts = [p.strip() for p in line.split('#')]
        if len(cell_parts) >= 4:
            bms_data['cells'].append({'id': i + 1, 'voltage': safe_float(cell_parts[0])/1000.0, 'temperature': safe_int(cell_parts[1])/1000.0, 'status_1': cell_parts[2], 'status_2': cell_parts[3]})
